...
const Achievements = () => {
  return (
    <section className='bg-primary text-slate-200 flex flex-col gap-4 p-4 md:p-8 md:py-4' id='achievements'>
      ...
      <div>
        <h3 className='text-slate-100 text-2xl'>Manchester United</h3>
        <div className='flex flex-wrap'>
      ...
      <div>
        ...
        <div>
        <h3 className='text-slate-100 text-2xl'>AL Nassr</h3>
        <div className='flex flex-wrap'>
          {achievements.filter((val) => val.type == 'club' && val.club == 'Al Nassr').map((acc, key) => {
            return (
              <RecordCard key={key} count={acc.count} title={acc.name} />
            )
          })}
